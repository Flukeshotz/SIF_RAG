...
# Domain specific semaphores mapping
domain_semaphores = {}
...
async def main():
    ...
    print(f"Starting V2 audit of {len(sources)} sources...")
    
    tasks = [audit_source(s) for s in sources]
    results = []
    
    # Process in batches to avoid OOM or global connection limits
    ...
    conn_error = sum(1 for r in results if r["error_type"] in ["connection_error", "too_many_requests"])
    
    with open("data/raw/live_audit_results_v2.json", "w") as f:
        json.dump(results, f, indent=2)
        
    print(f"Audit complete! Success rate: {(success/total)*100:.1f}%")
    
    # Generate the markdown report
    with open("docs/live_ingestion_report_v2.md", "w") as f:
        f.write("# Phase 2.6 — Live Ingestion Report V2\n\n")
        f.write("## Summary Metrics\n")
        f.write(f"- **Total Sources:** {total}\n")
        f.write(f"- **Successful Downloads:** {success} ({(success/total)*100:.1f}%)\n")
        ...
        f.write("## Detailed Source Audit\n\n")
        f.write("| Source ID | Organization | Status Code | Content Type | Success | Size (KB) |\n")
        f.write("|---|---|---|---|---|---|\n")
        for r in results:
            size_kb = f"{r['file_size']/1024:.1f}" if r['file_size'] else "0"
            status = r['status_code'] or r['error_type']
            success_str = "✅" if r['download_success'] else "❌"
            f.write(f"| {r['source_id']} | {r['organization']} | {status} | {r['content_type']} | {success_str} | {size_kb} |\n")
