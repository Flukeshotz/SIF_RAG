...
interface SideNavBarProps {
    onViewChange: (view: 'chat' | 'architecture' | 'insights' | 'scheduler' | 'coming_soon' | 'market_explorer') => void;
    currentView: string;
    isOpen?: boolean;
    onClose?: () => void;
}

const SideNavBar: React.FC<SideNavBarProps> = ({ onViewChange, currentView, isOpen = false, onClose }) => {
    return (
        <>
            ...
            <aside className={`fixed left-0 top-0 h-full z-40 pt-12 flex-col w-[280px] bg-surface-container-low border-r border-outline-variant transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'} md:translate-x-0 flex`}>
                <div className="p-margin flex flex-col gap-xs border-b border-outline-variant relative">
                    ...
                    {onClose && (
                        <button onClick={onClose} className="md:hidden absolute top-4 right-4 text-on-surface-variant hover:text-on-surface p-1">
                ...
                <div className="flex-1 overflow-y-auto px-sm py-sm flex flex-col gap-base">
                    ...
                    <button onClick={() => onViewChange('market_explorer')} className={`flex items-center gap-md px-md py-sm rounded-lg transition-all w-full text-left ${currentView === 'market_explorer' ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-variant'}`}>
                        <span className="material-symbols-outlined text-[20px]">account_balance</span>
                        <span className="font-label-md text-label-md">Market Explorer</span>
                    </button>
                    <button onClick={() => onViewChange('coming_soon')} className={`flex items-center gap-md px-md py-sm rounded-lg transition-all w-full text-left ${currentView === 'coming_soon' ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-variant'}`}>
